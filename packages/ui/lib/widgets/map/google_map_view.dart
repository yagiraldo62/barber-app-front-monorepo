...
class _GoogleMapViewState extends State<GoogleMapView> {
  ...
  void setMapStyle() {
    if (darkMapStyle != null &&
      ...
      mapController!.setMapStyle(
        themeManager.isDark ? darkMapStyle : lightMapStyle,
      );
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      //     center =
      //         pos != null
      //             ? LatLng(pos['latitude'] ?? 0.0, pos['longitude'] ?? 0.0)
      //             : null;
      //   });
      //   onBuildComplete();
      // });
    ...
    rootBundle.loadString('assets/map_styles/light_map_style.txt').then((
      string,
    ) {
      lightMapStyle = string;
      setMapStyle();
    });
    rootBundle.loadString('assets/map_styles/dark_map_style.txt').then((
      string,
    ) {
      darkMapStyle = string;
      setMapStyle();
  ...
  Widget build(BuildContext context) {
    List<dynamic> markersRepaintBoundaries =
        markersResources
