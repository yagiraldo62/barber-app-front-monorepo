...
class ProfileFormController extends GetxController
  ...
  void onInit() {
    ...
    if (currentProfile != null) {
      ...
      slugController.text = currentProfile!.slug ?? '';
  ...
  Future<void> onSavedDefault(ProfileModel profile) async {
    ...
    Get.offAndToNamed(Routes.artistHome);
  }
  ...
  void nextStep() {
    switch (currentStep.value) {
      ...
      case ProfileFormStep.name:
        ...
        // Generate slug from name if it's a creation
        if (isCreation && slugController.text.isEmpty) {
          slugController.text = _generateSlugFromName(nameController.text);
        }
        // Reset animation state when successfully moving to next step
        animationsComplete.value = false;
        currentStep.value = ProfileFormStep.slug;
        break;
      case ProfileFormStep.slug:
        if (slugController.text.isEmpty) {
          Snackbars.error(message: 'El slug es requerido');
          return;
        }
        if (!_isValidSlug(slugController.text)) {
          Snackbars.error(
            message:
                'El slug solo puede contener letras, números, puntos y guiones',
          );
          return;
        }
        if (slugController.text.length < 3) {
          Snackbars.error(message: 'El slug debe tener al menos 3 caracteres');
          return;
        }
        // Reset animation state when successfully moving to next step
        animationsComplete.value = false;
      ...
      case ProfileFormStep.description:
        // Description is now optional, skip validation
        // Reset animation state when successfully moving to next step
        animationsComplete.value = false;
  ...
  /// Validate slug format - only alphanumeric, dots, and hyphens allowed
  bool _isValidSlug(String slug) {
    // Only allow alphanumeric characters, dots, and hyphens
    return RegExp(r'^[a-zA-Z0-9.\-]+$').hasMatch(slug);
  }

  /// Generate slug from name with format: name-with-dashes.randomNumber
  String _generateSlugFromName(String name) {
    // Convert to lowercase
    String slug = name.toLowerCase();
    // Remove special characters, keep only alphanumeric, spaces, dots and hyphens
