...
enum CreateProfileStep { profile, location, services, availability, members }
...
class SetupScopeController extends GetxController {
  ...
  void onProfileSaved(ProfileModel profile) async {
    ...
    if (profile.type == ProfileType.artist &&
      ...
      Get.offAllNamed(Routes.artistHome);
      return;
    }
  ...
  void onFinish() async {
    // Mark the members_up setup step as complete
    try {
      final locationProvider = Get.find<ArtistLocationProvider>();
      await locationProvider.markSetupStepComplete(
        currentProfile.value!.id!,
        currentLocation.value!.id!,
        step: LocationSetupStep.membersUp,
      );
    } catch (e) {
      Log('Error marking members_up as complete: $e');
    }
  ...
  void _buildSteps() {
    ...
    final profileRoutes = [Routes.setupProfile, Routes.createProfile];
    final locationRoutes = [Routes.setupLocation, Routes.createLocation];

    final creationRoutes = [Routes.createProfile, Routes.createLocation];
    ...
    if (profileType.value == ProfileType.organization ||
      ...
      if (currentProfile.value != null) {
        if (currentLocation.value?.id == null) {
          ...
          } else if (currentLocation.value!.membersUp == false &&
              profileType.value == ProfileType.organization) {
            currentStep.value = CreateProfileStep.members;
          } else {
            Get.offAllNamed(Routes.artistHome);
          }
        }
