...
class LocationFormController extends GetxController
  ...
  Future<void> _submitForm() async {
    try {
      ...
      late LocationModel result;
      final isUpdate = currentLocation != null && currentLocation!.id != null;

      if (isUpdate) {
        // Update existing location
        result = await provider.updateLocation(
          profileId,
          currentLocation!.id!,
        ...
        Snackbars.success(message: 'Ubicación actualizada exitosamente');
      } else {
        // Create new location
        result = await provider.createLocation(
          profileId,
          nameController.text,
